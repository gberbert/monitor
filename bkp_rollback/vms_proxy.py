...
def remove_event():
    day_key = request.json.get('day_key')
    database.delete_event(day_key)
    return jsonify({"status": "ok"})

@app.route('/api/event_today', methods=['GET'])
def get_event_today():
    import datetime
    now = datetime.datetime.now()
    key = f"{str(now.day).zfill(2)}/{str(now.month).zfill(2)}"
    evt = database.get_event_by_key(key)
    if evt:
        return jsonify(evt)
    return jsonify(None)
...
# --- GIPHY API PROXY ---
@app.route('/api/giphy_search')
def giphy_search():
    query = request.args.get('q', '')
    offset = request.args.get('offset', '0')
    api_key = None
    
    # Simple Secrets Reader
    try:
        with open('secrets.yml', 'r') as f:
            for line in f:
                if 'giphy_api_key' in line:
    ...
    if not api_key:
        return jsonify({"error": "No API Key configured in secrets.yml"}), 500
        
    url = f"https://api.giphy.com/v1/gifs/search?api_key={api_key}&q={query}&limit=24&offset={offset}&rating=g"
    try:
...
# --- PROXY PARA STREAMS DE VIDEO (RAIZ DO GO2RTC) ---
@app.route('/api/stream.mp4')
@app.route('/api/stream.mjpeg')
def proxy_video_stream():
    filename = request.path.split('/')[-1]
    target_url = f"{GO2RTC_API}/{filename}"
    
    # Anexa query string manualmente para garantir (ex: ?src=camera1)
    if request.query_string:
        target_url += '?' + request.query_string.decode('utf-8')

    # print(f"[VIDEO PROXY TARGET] {target_url}")
...
# --- PROXY PARA API DO GO2RTC (MODO ORIGINAL RESTAURADO) ---
@app.route('/api/<path:subpath>', methods=['GET', 'POST', 'PUT', 'DELETE'])
def proxy_api(subpath):
    target_url = f"{GO2RTC_API}/api/{subpath}"
    # Debug silencioso para nao poluir
